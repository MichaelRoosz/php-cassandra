...
final class SetCollection extends ValueReadableWithoutLength {
    ...
    public static function fromMixedValue(mixed $value, ?TypeInfo $typeInfo = null): static {
        if (!is_array($value)) {
            throw new Exception('Invalid set value; expected array', ExceptionCode::TYPE_SET_INVALID_VALUE_TYPE->value, [
                'value_type' => gettype($value),
            ]);
        }

        if ($typeInfo === null) {
            throw new Exception('typeInfo is required', ExceptionCode::TYPE_SET_TYPEINFO_REQUIRED->value);
        }

        if (!$typeInfo instanceof SetCollectionInfo) {
            throw new Exception('Invalid type info, SetCollectionInfo expected', ExceptionCode::TYPE_SET_INVALID_TYPEINFO->value, [
