#!/usr/bin/env bash
#
# Validate the pure-PHP Lz4Decompressor against a proven external LZ4 tool.
#
# This is a thin convenience wrapper around validate-decompressor.php. Its only
...
#
# Usage:
#   scripts/lz4/validate-decompressor.sh [--download-corpus] [extra path ...]
#
# Any non-flag arguments are passed straight through to the PHP validator, so
# you can point it at your own files:
#   scripts/lz4/validate-decompressor.sh /path/to/silesia
#
set -euo pipefail
...
exec "${PHP_BIN}" "${SCRIPT_DIR}/validate-decompressor.php" "${extra_args[@]}"
