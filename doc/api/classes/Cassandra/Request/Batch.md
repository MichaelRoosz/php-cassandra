...
* Full name: `\Cassandra\Request\Batch`
...
```php
public __construct(\Cassandra\Request\BatchType $type = \Cassandra\Request\BatchType::LOGGED, \Cassandra\Consistency $consistency = \Cassandra\Consistency::ONE, \Cassandra\Request\Options\BatchOptions $options = new \Cassandra\Request\Options\BatchOptions()): mixed
```
...
```php
public appendPreparedStatement(\Cassandra\Response\Result\PreparedResult $prepareResult, array $values = []): self
```
...
| Parameter        | Type                                          | Description |
|------------------|-----------------------------------------------|-------------|
| `$prepareResult` | **\Cassandra\Response\Result\PreparedResult** |             |
| `$values`        | **array**                                     |             |
...
```php
public appendQuery(string $query, array $values = []): self
```
...
```php
public getOptions(): \Cassandra\Request\Options\BatchOptions
```
