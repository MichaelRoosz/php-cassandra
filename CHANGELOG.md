...
* Request timeouts: `Cassandra\Connection\ConnectionOptions::$requestTimeoutInSeconds` (default 30s) bounds how long the server may take to answer, raising the new `Cassandra\Exception\RequestTimeoutException`. Set it per request via `requestTimeoutInSeconds` on `QueryOptions`/`ExecuteOptions`/`BatchOptions`/`PrepareOptions`, for everything that follows via `Cassandra\Connection::setRequestTimeout()`, or for a single call via `syncRequest()` / `asyncRequest()`. The most specific one wins.
* `Cassandra\Connection::asyncRequest()` takes a `$requestTimeoutInSeconds` override, so an async statement can be given its own budget without going through request options.
...
* Wait bounds: every `waitFor…()` method takes `$timeoutInSeconds` limiting how long that call blocks — `0` returns as soon as there is nothing more to read, `INF` waits indefinitely, `null` uses the method's own default. A wait never blocks on the transport once its deadline has passed, so a bound of `0` and a request that has already run out are both reported at once rather than after another receive timeout.
