...
* `Cassandra\Connection`: waiting on a statement that this connection does not know — one from another `Connection`, or left over from before this one was replaced — spun at full CPU forever when the wait was unbounded, since nothing arriving on this socket could ever retire that statement's deadline. It is now rejected at once with a `Cassandra\Exception\StatementException`.
