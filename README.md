...
- v3/v4: a `USE` is sent, so the keyspace is a property of the node's session and holds for every request on the connection. A keyspace that does not exist fails the call that set it, and leaves the connection on the keyspace it was already on.
