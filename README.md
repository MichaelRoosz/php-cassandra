...
Note that a deadline is only noticed once the read the client is blocked in returns, so against a *completely silent* server the transport timeout is the granularity of deadline detection: with the defaults, a request timeout fires somewhere between 30s and 45s. Lower `SO_RCVTIMEO` / `timeoutInSeconds` if you need tighter deadlines. A deadline that has already passed never buys another blocking read, so a request that ran out while you were busy elsewhere is reported at once rather than after another receive timeout, and a connection busy answering other requests cannot defer it either.
