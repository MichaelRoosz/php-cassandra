...
final class DataTypeRoundtripNativeInputTest extends AbstractIntegrationTestCase {
    ...
    public function testVectorRoundtrip(): void {
        ...
        foreach ($testValues as $index => $testValue) {
            $this->connection->query(
                "INSERT INTO {$this->keyspace}.test_vector_smallint4 (id, value) VALUES (?, ?)",
                [$index, $testValue],
            );

            $result = $this->connection->query(
                "SELECT value FROM {$this->keyspace}.test_vector_smallint4 WHERE id = ?",
                [$index]
            )->asRowsResult();
        ...
        foreach ($testValues as $index => $testValue) {
            $this->connection->query(
                "INSERT INTO {$this->keyspace}.test_vector_text2 (id, value) VALUES (?, ?)",
                [$index, $testValue],
            );

            $result = $this->connection->query(
                "SELECT value FROM {$this->keyspace}.test_vector_text2 WHERE id = ?",
                [$index]
            )->asRowsResult();
